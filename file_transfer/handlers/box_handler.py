"""Copy files to the local Box Drive sync folder.

Requires the Box Drive desktop client to be installed and syncing.
...
class BoxHandler(BaseHandler):
    ...
    def __init__(self, handler_config: dict[str, Any] | None = None) -> None:
        ...
        self.sync_folder = Path(os.getenv("BOX_SYNC_FOLDER", ""))

    # ------------------------------------------------------------------
    def validate_credentials(self) -> bool:
        if not str(self.sync_folder):
            logger.error("[box] BOX_SYNC_FOLDER must be set in .env")
            return False
        if not self.sync_folder.is_dir():
            logger.warning(
                "[box] Sync folder does not exist yet: %s  "
    ...
    def transfer(self, file_path: Path) -> None:
        ...
        logger.info("[box] Copied to sync folder: %s", dest)
